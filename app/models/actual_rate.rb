class ActualRate < Rate
  class << self
    ...
    def get_moving_average(pair, interval)
      ...
      [].tap do |arr|
        open_rates = ActualRate.pair(pair).interval(interval).order('to_date DESC').select('to_date, open').limit(174)
        (open_rates.size - 74) < 0 ? [] : (open_rates.size - 74).times do |i|
          arr << {
            :time => (open_rates[i].to_date + 1 + 9 * 60 * 60).strftime('%Y-%m-%d %H:%M:%S'),
