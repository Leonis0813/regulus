...
describe AnalysesController, type: :controller do
  ...
  shared_context 'リクエスト送信' do |params: default_params|
    before do
      response = put(:upload_result, params: params)
  ...
  describe '異常系' do
    ...
    context 'modelの型が不正な場合' do
      ...
      include_context 'リクエスト送信', params: {model: 'invalid'}
      it_behaves_like 'レスポンスが正しいこと', status: 400, body: {'errors' => errors}
    end

    context 'modelのファイルが不正な場合' do
      params = {model: Rack::Test::UploadedFile.new(File.open(invalid_file_path))}
      errors = [{'error_code' => 'invalid_param_model'}]
